...
Sentry._init=Sentry.init,Sentry.init=function(a){var b,c,d,e,f,g,h;if(!a.dsn)return Sentry._init(a),void 0;if(b=a.dsn.replace(/(https:)|(http:)/,document.location.protocol).replace(/@.*com?/,"@"+document.domain+"/apmfe"),a.dsn=b,a.sampleRate=1,c=window.okGlobal?window.okGlobal.envSign:"",c&&(a.environment=c),d=RegExp(/daily/),!c.match(d)){Sentry._init(a);try{e=localStorage.getItem("ok_global"),f=e?JSON.parse(e):{},g=f.profile||{},h=g.uuid,h&&Sentry.setUser({id:h})}catch(i){}}};
